...
/* ─── STATIC AGENT PROFILES (descriptions/skills only — status comes from Supabase) ─── */
const AGENT_PROFILES = {
  zoriya: { color:T.blueLight, initials:"ZO", skills:["Orchestration","Memory","Planning","Delegation"], description:"Coordinates all operations. Manages memory, task delegation, and system health across the ZORIYA AI stack.", x:30, y:40 },
  vector: { color:T.green,     initials:"VE", skills:["Research","Analysis","Scoring","Contracts"],       description:"Scans the web daily for high-value B2B IT contract opportunities. Scores, categorizes, and surfaces the best matches.", x:65, y:35 },
};

const CRON_JOBS = [
  { id:1, name:"Contract Scan",       agent:"Vector", schedule:"Daily @ 6:00 PM",  next:"Today 6:00 PM",    color:T.green,     status:"scheduled" },
  { id:2, name:"Morning Brief",       agent:"Zoriya", schedule:"Daily @ 8:00 AM",  next:"Tomorrow 8:00 AM", color:T.blueLight, status:"scheduled" },
  ...
  { id:5, name:"Weekly Summary",      agent:"Vector", schedule:"Sun @ 9:00 AM",    next:"Sun Mar 15",       color:T.red,       status:"scheduled" },
];

const INITIAL_MESSAGES = [
  { id:1, from:"Zoriya", text:"Connected to Mission Control. Type a command or question to interact with your agents.", time:"Now", type:"agent" },
];
...
const agentColor = name => AGENT_PROFILES[name?.toLowerCase()]?.color ?? T.blueLight;
  ...
  return (
    <aside style={{ width:210, minHeight:"100vh", flexShrink:0, background:T.surface, borderRight:`1px solid ${T.border}`, display:"flex", flexDirection:"column", position:"relative", zIndex:10 }}>
      ...
      <div style={{ padding:"12px 12px 16px", borderTop:`1px solid ${T.border}` }}>
        ...
        {Object.entries(AGENT_PROFILES).map(([id,p]) => { const a = {id, name:id.charAt(0).toUpperCase()+id.slice(1), ...p}; return (
...
/* ─── HQ VIEW ────────────────────────────────────────────────────────────── */
function HQView({ live }) {
  live = live || useLiveData();
    ...
    <div style={{ padding:28, animation:"fadeUp .3s ease" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:24 }}>
        {[
          ...
          { label:"Tasks This Week",  val: live.stats?.tasks_this_week ?? "—", color:T.blueLight },
        ...
        <Card style={{ padding:0, overflow:"hidden" }}>
          {live.activity.length===0 && (
            <div style={{ padding:"28px 18px", textAlign:"center" }}>
              <p style={{ color:T.textMuted, fontSize:12, fontFamily:"'Geist Mono',monospace" }}>No activity yet — run Vector or trigger an agent action</p>
            </div>
          )}
          {live.activity.map((e,i,arr)=>(
            <div key={e.id} className="row-item" style={{ display:"flex", gap:14, padding:"14px 18px", borderBottom:i<arr.length-1?`1px solid ${T.border}`:"none" }}>
              ...
              <div style={{ flex:1 }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3 }}>
                  ...
                  <span style={{ fontSize:11, color:T.textMuted, fontFamily:"'Geist Mono', monospace" }}>{new Date(e.created_at).toLocaleTimeString()}</span>
                </div>
    ...
    <div style={{ padding:28, animation:"fadeUp .3s ease" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 300px", gap:20 }}>
        ...
        <Card style={{ padding:0, overflow:"hidden", height:480, position:"relative" }}>
          ...
          {/* Agents */}
          {Object.entries(AGENT_PROFILES).map(([id,p])=>{ const a={id,name:id.charAt(0).toUpperCase()+id.slice(1),x:p.x??30,y:p.y??60,...p}; return (
            <div key={a.id} onClick={()=>setSelected(selected?.id===a.id?null:a)} style={{ position:"absolute", left:`${a.x}%`, top:`${a.y}%`, transform:"translate(-50%,-50%)", cursor:"pointer", animation:"float 4s ease-in-out infinite", animationDelay:a.id==="vector"?"1.8s":"0s", zIndex:2 }}>
              ...
              <div style={{ position:"absolute", bottom:"100%", left:"50%", transform:"translateX(-50%)", marginBottom:8, whiteSpace:"nowrap", background:T.surfaceHi, border:`1px solid ${T.border}`, borderRadius:8, padding:"4px 9px", fontSize:10, color:T.textSecondary, animation:"fadeIn .4s ease", pointerEvents:"none" }}>
                {AGENT_ACTIVITIES[id]?.[actIdx[id]] || 'Standby...'}
              </div>
            </div>
          ); })}
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          {Object.entries(AGENT_PROFILES).map(([id,p])=>{ const a={id,name:id.charAt(0).toUpperCase()+id.slice(1),...p}; return (
            <Card key={a.id} className="card" onClick={()=>setSelected(selected?.id===a.id?null:a)} style={{ padding:14, cursor:"pointer", border:selected?.id===a.id?`1px solid ${a.color}44`:undefined }}>
              ...
              <p style={{ fontSize:11, color:T.textMuted, fontFamily:"'Geist Mono', monospace", marginBottom:10 }}>{a.description?.slice(0,50)||""}</p>
              {/* Mini activity bar */}
              <div style={{ height:3, borderRadius:2, background:"rgba(255,255,255,0.06)", overflow:"hidden" }}>
                <div style={{ height:"100%", width:`${id==="zoriya"?68:42}%`, background:a.color, borderRadius:2, transition:"width 1s ease" }} />
              </div>
              <div style={{ display:"flex", justifyContent:"space-between", marginTop:5 }}>
                <span style={{ fontSize:9, color:T.textMuted, fontFamily:"'Geist Mono', monospace" }}>{id==="zoriya"?"68":"42"}% active</span>
                <span style={{ fontSize:9, color:T.textMuted, fontFamily:"'Geist Mono', monospace" }}>agent online</span>
              </div>
            </Card>
          ); })}
        </div>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0 }}>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"24px 24px 8px" }}>
          ...
          {channelMsgs.map((msg,i)=>{
            ...
            const agentProfile = AGENT_PROFILES[msg.from?.toLowerCase()];
...
function TasksView() {
  ...
  const [newAgent, setNewAgent] = useState("Zoriya");
  const cols = ["Backlog","In Progress","Done"];

  const addTask = () => {
    if(!newTask.trim()) return;
    setTasks(prev=>[...prev,{id:Date.now(),title:newTask,status:"Backlog",priority:"medium",agent:newAgent,project:"General"}]);
    setNewTask("");
  };
  ...
  return (
    <div style={{ padding:28, animation:"fadeUp .3s ease" }}>
      ...
      <div style={{ display:"flex", gap:24, marginBottom:24 }}>
        {[{label:"Open",val:tasks.filter(t=>t.status!=="Done").length,color:T.blueLight},{label:"In Progress",val:tasks.filter(t=>t.status==="In Progress").length,color:T.amber},{label:"Total",val:tasks.length,color:T.textSecondary},{label:"Done",val:tasks.filter(t=>t.status==="Done").length,color:T.green}].map(s=>(
      ...
      <Card style={{ padding:16, marginBottom:20, display:"flex", gap:10 }}>
        <input value={newTask} onChange={e=>setNewTask(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addTask()} placeholder="Add a new task…" style={{ flex:1, padding:"8px 12px", borderRadius:8, background:"rgba(255,255,255,0.04)", border:`1px solid ${T.border}`, color:T.textPrimary, fontSize:13, outline:"none" }} />
        <select value={newAgent} onChange={e=>setNewAgent(e.target.value)} style={{ padding:"8px 12px", borderRadius:8, background:"rgba(255,255,255,0.04)", border:`1px solid ${T.border}`, color:T.textPrimary, fontSize:13, outline:"none" }}>
          {Object.keys(AGENT_PROFILES).map(id=><option key={id} value={id.charAt(0).toUpperCase()+id.slice(1)}>{id.charAt(0).toUpperCase()+id.slice(1)}</option>)}
        </select>
        <button onClick={addTask} style={{ padding:"8px 16px", borderRadius:8, background:T.blue, border:"none", color:"#fff", fontSize:13, fontWeight:600, cursor:"pointer" }}>Add</button>
      </Card>
                ...
                <Card key={task.id} style={{ padding:14 }}>
                  ...
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap", justifyContent:"space-between" }}>
                    ...
                    <div style={{ display:"flex", gap:4 }}>
                      {col!=="In Progress" && <button onClick={()=>setTasks(p=>p.map(t=>t.id===task.id?{...t,status:"In Progress"}:t))} style={{ fontSize:10, padding:"2px 6px", borderRadius:4, background:`${T.amber}18`, border:`1px solid ${T.amber}33`, color:T.amber, cursor:"pointer" }}>→ WIP</button>}
                      {col!=="Done" && <button onClick={()=>setTasks(p=>p.map(t=>t.id===task.id?{...t,status:"Done"}:t))} style={{ fontSize:10, padding:"2px 6px", borderRadius:4, background:`${T.green}18`, border:`1px solid ${T.green}33`, color:T.green, cursor:"pointer" }}>✓</button>}
                    </div>
                  </div>
                </Card>
              ))}
...
function ProjectsView() {
  const [projects, setProjects] = useState([
    { id:1, name:"Mission Control", status:"Active", progress:78, agent:"Zoriya", priority:"high", desc:"Live dashboard connected to Supabase with real agent data, contract finds, and remote commands.", tasks:"8/10" },
    { id:2, name:"Contract Intelligence", status:"Active", progress:65, agent:"Vector", priority:"high", desc:"AI-powered contract scanning. Vector scans daily, scores opportunities, and pushes results to Mission Control.", tasks:"6/9" },
    { id:3, name:"OpenClaw Infrastructure", status:"Active", progress:90, agent:"Zoriya", priority:"medium", desc:"Core system — gateway, bridge server, Supabase sync, auto-start on boot.", tasks:"9/10" },
  ]);
  return (
    <div style={{ padding:28, animation:"fadeUp .3s ease" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:16 }}>
        {projects.map(p=>{
          const prof = AGENT_PROFILES[p.agent?.toLowerCase()] || AGENT_PROFILES.zoriya;
          return (
            <Card key={p.id} style={{ padding:22 }}>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <div style={{ display:"flex", alignItems:"center", gap:7 }}>
                  <div style={{ width:22, height:22, borderRadius:6, background:`${prof.color}22`, border:`1px solid ${prof.color}44`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontWeight:700, color:prof.color }}>{prof.initials}</div>
...
/* ─── CONTRACTS ──────────────────────────────────────────────────────────── */
function ContractsView({ live }) {
  live = live || useLiveData();
  const [filter, setFilter] = useState("All");
  const [search, setSearch] = useState("");

  const contracts = live.contracts;
  const cats = ["All",...Array.from(new Set(contracts.map(c=>c.category||"General").filter(Boolean)))];
  const filtered = contracts.filter(c=>{
    const matchCat = filter==="All" || (c.category||"General")===filter;
    const matchSearch = !search || (c.title||c.url||"").toLowerCase().includes(search.toLowerCase()) || (c.agency||"").toLowerCase().includes(search.toLowerCase());
    return matchCat && matchSearch;
  });
  const avgScore = contracts.length ? Math.round(contracts.reduce((s,c)=>s+(c.match_pct||0),0)/contracts.length) : 0;
  const today = new Date().toDateString();
  const todayCount = contracts.filter(c=>new Date(c.found_at||c.created_at).toDateString()===today).length;

  const grouped = {};
  filtered.forEach(c=>{
    const d = c.found_date || c.found_at?.split("T")[0] || new Date().toLocaleDateString("en-CA");
    if(!grouped[d]) grouped[d]=[];
    grouped[d].push(c);
  });

  const scoreCol = pct => pct>=70?T.green:pct>=40?T.amber:T.red;

  return (
    <div style={{ padding:28, animation:"fadeUp .3s ease" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:22 }}>
        <div>
          <p style={{ fontSize:10, color:T.textMuted, letterSpacing:"0.14em", marginBottom:5, fontFamily:"'Geist Mono', monospace" }}>VECTOR · LIVE CONTRACT INTELLIGENCE</p>
          <h1 style={{ fontSize:24, fontWeight:700, color:T.textPrimary, letterSpacing:"-0.02em" }}>Contract Finds</h1>
        </div>
        <div style={{ display:"flex", gap:20 }}>
          {[{l:"Total",v:live.loading?"…":contracts.length},{l:"Today",v:live.loading?"…":todayCount},{l:"Avg Score",v:live.loading?"…":avgScore+"%"}].map(s=>(
      ...
      {live.loading && <Card style={{ padding:32, textAlign:"center" }}><p style={{ color:T.textMuted, fontFamily:"'Geist Mono',monospace", fontSize:12 }}>Loading contracts from Supabase…</p></Card>}
      {!live.loading && contracts.length===0 && (
        <Card style={{ padding:40, textAlign:"center" }}>
          <p style={{ fontSize:28, marginBottom:12 }}>📡</p>
          <p style={{ color:T.textPrimary, fontWeight:600, marginBottom:8 }}>No contracts yet</p>
          <p style={{ color:T.textMuted, fontSize:13 }}>Go to HQ and click ▶ RUN VECTOR to trigger a scan</p>
        </Card>
      )}
      {!live.loading && contracts.length>0 && (<>
        <div style={{ display:"flex", gap:10, marginBottom:22, flexWrap:"wrap" }}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search contracts…" style={{ flex:1, minWidth:220, padding:"8px 14px", borderRadius:8, background:"rgba(255,255,255,0.04)", border:`1px solid ${T.border}`, color:T.textPrimary, fontSize:13, outline:"none" }} />
        ...
        {Object.entries(grouped).sort(([a],[b])=>b.localeCompare(a)).map(([date,items])=>(
          <div key={date} style={{ marginBottom:26 }}>
            <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:12 }}>
              <p style={{ fontSize:10, color:T.textMuted, letterSpacing:"0.1em", whiteSpace:"nowrap", fontFamily:"'Geist Mono', monospace" }}>{date.toUpperCase()}</p>
              <div style={{ flex:1, height:1, background:T.border }} />
              <p style={{ fontSize:10, color:T.textMuted, fontFamily:"'Geist Mono', monospace" }}>{items.length} FINDS</p>
            </div>
            <Card style={{ padding:0, overflow:"hidden" }}>
              {items.map((c,i)=>(
                <div key={c.id} className="row-item" style={{ display:"flex", alignItems:"center", gap:16, padding:"14px 18px", borderBottom:i<items.length-1?`1px solid ${T.border}`:"none", cursor:"pointer" }} onClick={()=>c.url&&window.open(c.url,"_blank")}>
                  <div style={{ width:46, height:46, borderRadius:10, flexShrink:0, background:`${scoreCol(c.match_pct)}12`, border:`1px solid ${scoreCol(c.match_pct)}33`, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
                    <span style={{ fontSize:15, fontWeight:700, color:scoreCol(c.match_pct), fontFamily:"'Geist Mono', monospace", lineHeight:1 }}>{c.match_pct||0}</span>
                    <span style={{ fontSize:8, color:T.textMuted, letterSpacing:"0.06em" }}>SCORE</span>
                  </div>
                  <div style={{ flex:1, minWidth:0 }}>
                    <p style={{ fontSize:14, fontWeight:600, color:T.textPrimary, marginBottom:3, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{c.title||c.url}</p>
                    <p style={{ fontSize:12, color:T.textSecondary }}>{c.agency||c.category||"Contract Opportunity"}</p>
                    {c.summary && <p style={{ fontSize:11, color:T.textMuted, marginTop:3, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{c.summary}</p>}
                  </div>
                  <div style={{ display:"flex", alignItems:"center", gap:10, flexShrink:0 }}>
                    <Tag color={T.textMuted}>{c.category||"General"}</Tag>
                    {c.value && <span style={{ fontSize:14, fontWeight:700, color:T.textPrimary, fontFamily:"'Geist Mono', monospace" }}>{c.value}</span>}
                    <Tag color={T.blueLight}>New</Tag>
                  </div>
                </div>
              ))}
            </Card>
...
function MemoryView() {
  ...
  const [selected, setSelected] = useState(null);
  const entries = live.activity;
  const grouped = {};
  entries.forEach(e=>{
    const d = e.created_at ? e.created_at.split("T")[0] : "Today";
    if(!grouped[d]) grouped[d]=[];
    grouped[d].push(e);
  });
  return (
    <div style={{ padding:28, animation:"fadeUp .3s ease" }}>
      ...
      {entries.length===0 && (
        <Card style={{ padding:40, textAlign:"center" }}>
          <p style={{ fontSize:28, marginBottom:12 }}>🧠</p>
          <p style={{ color:T.textPrimary, fontWeight:600, marginBottom:8 }}>No memory entries yet</p>
          <p style={{ color:T.textMuted, fontSize:13 }}>Activity logs will appear here as agents run tasks</p>
        </Card>
      )}
      {entries.length>0 && (
        <div style={{ display:"grid", gridTemplateColumns:"260px 1fr", gap:16 }}>
          <Card style={{ padding:0, overflow:"hidden", height:"fit-content" }}>
            <div style={{ padding:"12px 14px", borderBottom:`1px solid ${T.border}` }}>
              <p style={{ fontSize:9, color:T.textMuted, letterSpacing:"0.12em", fontFamily:"'Geist Mono', monospace" }}>ACTIVITY LOG · {entries.length} ENTRIES</p>
            </div>
            {Object.entries(grouped).sort(([a],[b])=>b.localeCompare(a)).map(([date,dayEntries])=>(
              <div key={date}>
                <div style={{ padding:"8px 14px", background:"rgba(255,255,255,0.02)", borderBottom:`1px solid ${T.border}` }}>
                  <p style={{ fontSize:10, color:T.textMuted, fontFamily:"'Geist Mono', monospace" }}>{date}</p>
                </div>
                {dayEntries.map(e=>(
                  <div key={e.id} className="row-item" onClick={()=>setSelected(e)} style={{ padding:"11px 14px", cursor:"pointer", borderBottom:`1px solid ${T.border}`, background:selected?.id===e.id?T.blueDim:"transparent", borderLeft:selected?.id===e.id?`2px solid ${T.blue}`:"2px solid transparent" }}>
                    <div style={{ display:"flex", gap:7, alignItems:"center", marginBottom:3 }}>
                      ...
                      <span style={{ fontSize:12, fontWeight:500, color:selected?.id===e.id?T.blueLight:T.textPrimary }}>{e.title||e.message?.slice(0,40)||"Activity"}</span>
                    </div>
                    <p style={{ fontSize:10, color:T.textMuted, fontFamily:"'Geist Mono', monospace", marginLeft:13 }}>{new Date(e.created_at).toLocaleTimeString()}</p>
                  </div>
                ))}
              </div>
            ))}
          ...
          {selected && (
            <Card style={{ padding:24 }}>
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:6 }}>
                <p style={{ fontSize:11, color:T.textMuted, fontFamily:"'Geist Mono', monospace" }}>{new Date(selected.created_at).toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"})}</p>
                <span style={{ color:T.textMuted }}>·</span>
                <p style={{ fontSize:11, color:T.textMuted, fontFamily:"'Geist Mono', monospace" }}>{new Date(selected.created_at).toLocaleTimeString()}</p>
              </div>
              <h2 style={{ fontSize:20, fontWeight:700, color:T.textPrimary, marginBottom:10, letterSpacing:"-0.01em" }}>{selected.title||"Activity"}</h2>
              ...
              <p style={{ fontSize:14, color:T.textSecondary, lineHeight:1.75 }}>{selected.message||""}</p>
            </Card>
          )}
          {!selected && <Card style={{ padding:40, display:"flex", alignItems:"center", justifyContent:"center" }}><p style={{ color:T.textMuted, fontSize:13 }}>Select an entry to view details</p></Card>}
        </div>
      )}
    </div>
  );
    ...
    <div style={{ padding:28, animation:"fadeUp .3s ease" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
        ...
        <Card style={{ padding:20 }}>
          ...
          {Object.entries(AGENT_PROFILES).map(([id,p],i,arr)=>{ const a={id,name:id.charAt(0).toUpperCase()+id.slice(1),...p}; return (
            <div key={a.id} style={{ marginBottom:i<arr.length-1?16:0 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
              ...
              <div style={{ height:3, borderRadius:2, background:"rgba(255,255,255,0.06)", overflow:"hidden" }}><div style={{ height:"100%", width:`${id==="zoriya"?68:42}%`, background:a.color, borderRadius:2 }} /></div>
              <p style={{ fontSize:10, color:T.textMuted, marginTop:4, fontFamily:"'Geist Mono', monospace" }}>{id==="zoriya"?"68":"42"}% CPU</p>
            </div>
          ); })}
        </Card>
      </div>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ display:"flex", gap:5, flexWrap:"wrap", alignItems:"center" }}>
                      ...
                      <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:5 }}>
                        <div style={{ width:5, height:5, borderRadius:"50%", background:agentColor(opp.agent) ?? ARB_GOLD }} />
...
export default function App() {
  ...
  const VIEWS = {
    ...
    contracts: <ContractsView live={live} />,
    cron:      <CronView />,
    leads:     <LeadsView />,
    capsule:   <CapsuleView />,
    broker:    <BrokerView />,
